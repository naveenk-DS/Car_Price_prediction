...
# -------------------------------
model_path = r"E:\Car_Price_Prediction\Car_Price_prediction\model.pkl"  # Change if needed
with open(model_path, "rb") as f:
    model = pickle.load(f)  # ✅ Loaded trained model object
...
# -------------------------------
# Input Fields (match training data columns)
# -------------------------------
...
Body_Type_numeric = st.selectbox("Body Type (Encoded)", [0, 1, 2, 3])  # Replace with actual encoding if available
Fuel_Type_numeric = st.selectbox("Fuel Type (Encoded)", [0, 1, 2, 3])  # Replace with actual encoding if available
Transmission_numeric = st.selectbox("Transmission Type (Encoded)", [0, 1])  # Replace with actual encoding if available
...
if st.button("Predict Price"):
    try:
        # Create DataFrame with the exact feature names used in training
        input_data = pd.DataFrame([{
            "Mileage_km": Mileage_km,
            "Number ownwer": Number_owner,  # Keep exact spelling as in training dataset
            "Mileage": Mileage,
            "Engine": Engine,
            "Max Power": Max_Power,
            "Torque": Torque,
